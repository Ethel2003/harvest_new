#!/usr/bin/env python3
"""
Script de test pour l'endpoint ajouter_membre_departement
Teste l'association de plusieurs membres à un département
"""
...
SERVITEURS_URL = f"{BASE_URL}/serviteurs"
MEMBRES_URL = f"{BASE_URL}/membres"
DEPARTEMENTS_URL = f"{BASE_URL}/departements"
...
def get_existing_ids():
    ...
    try:
        ...
        response = requests.get(f"{DEPARTEMENTS_URL}/", headers=HEADERS)
        if response.status_code == 200:
            departements_data = response.json()
            departement_ids = [dept['id'] for dept in departements_data.get('results', [])[:2]]
        else:
            departement_ids = [1, 2]  # Fallback
        
        return membre_ids, departement_ids
    except Exception as e:
        print(f"Erreur lors de la récupération des IDs: {e}")
        return [1, 2, 3], [1, 2]

def test_association_membres_departement():
    """Test 1: Association de plusieurs membres à un département"""
    print("\n1️⃣ Test ASSOCIATION_MEMBRES_DEPARTEMENT - Association normale")
    
    membre_ids, departement_ids = get_existing_ids()
    departement_id = departement_ids[0]  # Utiliser le premier département
    ...
    try:
        response = requests.post(f"{SERVITEURS_URL}/ajouter_membre_departement/", 
        ...
        if response.status_code == 201:
            ...
            print(f"Serviteurs créés: {data.get('data', {}).get('serviteurs_crees', 'N/A')}")
            print(f"Département associé: {data.get('data', {}).get('departement_associe', {}).get('nom', 'N/A')}")
            return True
        else:
...
def test_validation_departement_id_manquant():
    """Test 2: Validation - departement_id manquant"""
    print("\n2️⃣ Test VALIDATION - departement_id manquant")
...
def test_validation_membre_ids_manquant():
    ...
    _, departement_ids = get_existing_ids()
    departement_id = departement_ids[0]
...
def test_validation_departement_inexistant():
    """Test 5: Validation - département inexistant"""
    print("\n5️⃣ Test VALIDATION - Département inexistant")
    
    membre_ids, _ = get_existing_ids()
    
    payload = {
        "departement_id": 99999,  # ID inexistant
        "membre_ids": membre_ids
    }
...
def test_reponse_detaille():
    ...
    try:
        ...
        if response.status_code == 201:
            ...
            print(f"Message: {data.get('message', 'N/A')}")
            print(f"Serviteurs créés: {data.get('data', {}).get('serviteurs_crees', 'N/A')}")
            print(f"Serviteurs supprimés: {data.get('data', {}).get('serviteurs_supprimes', 'N/A')}")
            print(f"Département associé: {data.get('data', {}).get('departement_associe', {}).get('nom', 'N/A')}")
...
def main():
    """Fonction principale"""
    print("🧪 TEST AJOUTER_MEMBRE_DEPARTEMENT - Association membres-département")
    print("=" * 75)
    ...
    if test_association_membres_departement():
        success_count += 1
    
    # Test 2: Validation departement_id manquant
    if test_validation_departement_id_manquant():
        success_count += 1
    ...
    # Test 5: Validation département inexistant
    if test_validation_departement_inexistant():
        success_count += 1
